...
export const protectRoute = async(req,res,next) => {
    try{
        const token = req.cookies.jwt; // "jwt" is the name we used while creating token 
        ...
        const user = await User.findById(decoded.userId).select("-password"); // inside decoded we have userId - excluding password from the user object
